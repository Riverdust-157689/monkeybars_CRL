...
> **M4 状态更新（§9.51）**：删 `max(c_L,c_R)` 失败的原因已定量定位——它是目标里**增益最高**的一维（同样 3.2 cm 抓握座偏移，折算 0.226；米制位置维只有 0.0012），且 #17 的策略确实把它从 0.52 优化到 0.96–0.98；#18 回放显示它 82% 时间两手都不在杆上（松手坠落）。按用户提议改为**每只手 × 指令目标杆**的接触 （变体 `dual_cnext`，12 维），既保住承重项又消除「任一只手/任意一根杆」的退化；阈值须用 0.8。run #19 侦察命令见 §9.51.4。
