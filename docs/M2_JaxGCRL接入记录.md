...
- 两个接触项用的核宽是 **`TAU_NEXT = 5 cm / √ln2 = 0.0601`**，即"软接触在环境自己的抓握判据（5 cm）处正好 0.5"。通用 `TAU_CONTACT = 0.04` 的 0.5 点在 3.3 cm——**在包络里面**，稳定悬挂（抓手点漂到 3–4.5 cm）只能读到 ~0.55，goal 会**构造性不可达**。只改 `advance` 的这两项，旧变体仍用 `TAU_CONTACT` ⇒ run #7–#11 可比。
