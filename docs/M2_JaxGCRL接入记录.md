...
### 9.79 `support_dual_load_both`（bar-agnostic load 三元，冷启 6 h）：**第一次真正完成 B0→B2**

`runs/brach_lboth_2bar_6h/`（`--goal-variant support_dual_load_both`，14 维，`git 29c52e6`，
课程/配方与 6 h 基线逐项相同（`gmin 1 gmax 2 start_bar_max 1`，`--eval-goal-bar 1`，`expl_hold 10`），
50.13 M 步 / 60 evals，**冷启**（`init_from = None`）。）

#### 9.79.1 曲线：起飞晚（≈40 M 步）且**后期振荡/回退**

| eval | len | fell | max_bar | bar_R | advance_max | succ | ctL | nrL | hvL |
|---|---|---|---|---|---|---|---|---|---|
| 40 | 185.4 | 1.00 | 3.0 | −20.1 | **0.0** | 0 | 0.4 | 0.6 | 0.2 |
| 48 | 232.6 | 0.81 | 102.6 | +16.6 | 60.1 | 0.7 | 3.4 | 16.4 | 13.0 |
| **55** | **501.0** | **0.00** | **350.7** | **+302.2** | **323.1** | **127.5** | 211.1 | 216.5 | 5.4 |
| 57 | 458.7 | 0.12 | 310.8 | +256.1 | 283.9 | 31.2 | 54.4 | 59.3 | 4.9 |
| 59 | 283.6 | 0.69 | 123.6 | +36.3 | **98.8** | 83.9 | 90.5 | 92.0 | 1.5 |

⇒ 起飞在 **eval 46–49（约 39–41 M 步）**，峰值在 **eval 55（≈46 M 步，`len 501 / fell 0 / adv 323`）**，
之后**回退**到 eval 59（`adv 98.8 / fell 0.69`）——与 #21 `dual_hnext` 的后期回退是同一形态。
**所以"最终 checkpoint"不是这轮的最好状态**（这点下面两张渲染的差别会证实）。
另一个代价：起飞比 11 维基线晚一倍（基线 eval 28 就已 `len 501 / fell 0`）。

#### 9.79.2 B2 探针：**首次出现"两手真的挂在 B2 上"**

`--goal-bar 2`，4 episodes × 501 步，存活期（`两手同在 B2 最长` = 两手同时落在 B2 窗内的最长连续步数）：

| checkpoint | ep0 | ep1 | ep2 | ep3 |
|---|---|---|---|---|
| **基线 `support_dual` @50 M** | 57 步，0 步 | 60，0 | 61，0 | 58，0 |
| **`load_both` 峰值 @46.8 M** | **502 步**，承力 L39/R159，**同在 B2 33 步** | 238（min d 0.020）| 282（min d 0.037）| **502 步**，L16/R236，**同在 B2 22 步** |
| **`load_both` 末态 @50.1 M** | **502 步**，承力 L134/R266，**同在 B2 83 步** | 194（0.067）| 231（0.109）| 159（0.303）|

* **基线在 4/4 个 episode 里"两手同时落在 B2"的步数都是 0** —— 它只会用一只手在 B2 附近蹭（`min d 0.001–0.028`
  但从不承力）；两次 warm start 也一样（§9.77/§9.78）。
* 这一版：**峰值 checkpoint 有 2/4 个 episode 活满 501 步并把两手挂在 B2 上 22–33 个连续步**（右手在 B2 上
  真承力 159–236 步、载荷 63.6 N·m 量级），躯干末端停在 **x = +0.72**（B2 的悬挂位 ≈ +0.685）——**真的挂在 B2 下面**。
  末态 checkpoint 的 ep0 更长：**同在 B2 83 个连续步**。
* 且**悬停为 0%**（B2 与 B1 两个探针里 `hover_L/hover_R` 都是 0–1%），`ctR ≡ nrR`（右手一进窗就承力）——
  §9.72 的接触判据在真实策略上一直成立。
* 代价：**双峰**。没解掉的那 2–3 个 episode 在 159–282 步内既不碰 B2（min d 0.02–0.30）也守不住别的，
  而基线是"每次都稳定地链到 B2 但守不住"。也就是把"稳定的失败"换成了"有时成功、有时更早失败"。

#### 9.79.3 B1 探针：峰值 checkpoint 仍然很好，末态 checkpoint 退化

* **峰值 @46.8 M**：eval 曲线本身就是 B1 探针 —— eval 55 `len 501 / fell 0 / bar_R +302 / adv 323 / succ 127.5`
  ⇒ **B1 与 B2 同时成立**（这是目前所有 run 里最好的一格）。
* **末态 @50.1 M** 的 B1 渲染：`fell = [159, 165, −1, 159]`（3/4 坠落）、`dist 2.317 → 1.448`、
  右手真承力 32%、B1 窗内 L20%/R34% ⇒ **比 6 h 基线（4/4 存活、dist 0.038）差很多**。
  即后期回退把 B1 也带下去了。

#### 9.79.4 结论

1. **用户的判断成立**：把接触项从"指定杆"改成**与杆无关**，就同时避开了
   ① §9.75 的"扑 B2 跳 B1"（`support_dual_contact`）和 ② 距离判据的"贴杆不抓"。
   这是四条 B2 路线（基线 / 两次 warm start / bar-specific contact）里**唯一真正完成并抱住 B0→B2 的**。
2. **下一步应先解决"后期回退 + 双峰"**，而不是再加新维度：
   * 峰值在 ≈46 M 步，末态更差 ⇒ **checkpoint 选择**本身就是结果的一部分；
   * 最便宜的动作：从**峰值 checkpoint**（`runs/ckpt_lboth_2bar_6h/actor_046798592.pkl`）warm start 再跑 1–3 h，
     看它能不能在峰值附近稳住（warm start 路径已验证可用）；
   * 想让以后每个 checkpoint 都能 warm start（而不只是最后一个），需要让 `--save-every` 也存
     `(alpha, actor, critic)` 三元组（现在只有最后一个是三元组，中间只有 actor）——约 10 行改动；
   * 或者从优化稳定性下手（更小 lr / 更大 batch）验证回退是否纯优化问题。

### 9.80 ⚠️ 更正 §9.79：那次"成功"是**穿模**撑出来的（用户看录像发现）

用户看渲染后的两句观察，**都成立，而且第二句推翻了 §9.79 的结论**：
「全程单手完成任务，另一只手只是摆来摆去」「负责挂杆的手**与杆穿模**了」。

#### 9.80.1 度量方法与标定（新增守卫 `src/check_grasp_penetration.py`）

杆是 **CAPSULE**（半径 25 mm）、手是 **MESH/BOX**（每只手 22 个 geom，10 个 body）。物理跑在 MJX-Warp，
但录像与几何检查都在**原生 MuJoCo** 上做（同一 qpos），所以用 `mj_geomDistance` 量**有符号距离**
（>0 = 在外面，<0 = 穿透）。标定行（`--reference`）：

| 状态 | 最小有符号距离 |
|---|---|
| **`hang` 关键帧（人工标定过的双手悬挂）** | L **−5.1 mm** / R **−5.0 mm** —— 这就是"合法抓握"的量级（MuJoCo 软接触允许几 mm） |

#### 9.80.2 实测：右手的碰撞体**穿过**了杆，左手根本没碰杆

`load_both` 末态 @50.1 M，`--goal-bar 2`，4 episodes × 501 步（存活期/全段）：

| ep | 左手 min / 中位 / 步数<−10 mm | 右手 min / 中位 / 步数<−10 mm | 载荷 EMA L / R |
|---|---|---|---|
| 0 | −11.7 mm / **+110.7 mm** / 2 | **−48.9 mm** / −9.1 mm / **214** | 15.9 / 65.5 |
| 1 | −8.5 / **+200**(封顶,即>20 cm) / 0 | −35.2 / +200 / **144** | 27.9 / 52.1 |
| 2 | −20.8 / +200 / 3 | **−49.6** / +200 / **182** | 26.5 / 53.0 |
| 3 | −8.0 / +200 / 0 | −36.9 / +200 / **126** | 29.8 / 50.0 |

* **杆的半径只有 25 mm，右手却进去了最多 49.6 mm** —— 等于**整个手穿过去**；而且 502 步里有
  **126–214 步**（25–43%）超过 1 cm。
* **左手的中位距离是 +110 mm 到 >200 mm** —— 即**绝大多数步数根本不在杆附近**。用户说的"另一只手只是摆动"
  完全正确。
* ⇒ §9.79 里"两手同在 B2 最长 83 步"这个数字是**纯邻近判据（座到轴 <5 cm）**的产物：
  手的座可以在 5 cm 内而**手掌/手指并不在杆上**（甚至在杆里面）。**"完成 B0→B2"的结论撤回。**

#### 9.80.3 这不是新策略的问题：**6 h 基线的 B1 悬挂也在穿模**

`ckpt_dual2bar_6h` @goal-bar 1（它"4/4 存活 501 步、dist 0.038"的那次），250 步 × 4 eps：

| ep | 左手 min / 中位 / <−10 mm | 右手 min / 中位 / <−10 mm |
|---|---|---|
| 0 | −12.7 / +70.2 / 23 | **−33.8** / **−8.3** / 32 |
| 1 | −16.6 / +64.8 / 19 | −21.6 / −8.2 / 29 |
| 2 | −16.3 / +72.1 / 16 | −30.2 / −7.9 / 32 |
| 3 | −12.7 / +72.0 / 9 | −25.9 / −7.8 / 38 |

⇒ 基线**也是靠穿模挂住的**：右手在典型一步里就陷进去 **约 8 mm**、12–15% 的步数超过 1 cm，
而参考的关键帧只有 −5 mm。**所以"B0→B1 已经解决"这个结论也需要重新审视**（至少它的物理形式不合法）。

#### 9.80.4 原因（假设，指向求解器/接触参数而非策略）

1. `mujoco_warp` 在建模型时明确警告：**MULTICCD 开着时 `(CAPSULE, MESH)` 这一对最多生成 1 个接触**
   （`io.py:611-652`，`use_multiccd = (opt.disableflags & MULTICCD) == 0`）。手 vs 杆正是这一对。
2. 场景是 `ccd_iterations=35`（MuJoCo 默认）、`ls_iterations=20`（仓库为了速度**钉死**的值，
   M1 时定的），接触 `solref=[0.02, 1]`。**位置伺服的手在软接触 + 受限线搜索下可以把接触点推进去几厘米**。
3. 于是训练里"穿进杆里"是最省力的拿到 contact/载荷目标的方式 ⇒ 策略当然会用它。
   **我们的 `f`（手指载荷）在穿模状态下照样读 50–65 N·m**，所以它测不出这个问题。

#### 9.80.5 一个**独立**的次生问题：裸载荷不等于"碰到杆"

上表里左手在**离任何杆 11–20 cm** 时，`load`（手指 DOF `Σ|qfrc_constraint|` 的 EMA）均值仍有
**16–30 N·m**。也就是说这个原始量还包含**关节限位力/自碰撞**，不完全是杆接触。
（goal 里的 `f` 因为还要 AND 上 5 cm 的座距离门，所以不会单独由此误判——但**报告裸载荷时必须注明**。
`check_contact_sense.py` 的负对照当时用的是"手指全开"，恰好避开了这一档，应当补一条
"手指半闭但悬空"的对照。）

#### 9.80.6 下一步：先修物理，再谈目标

已给探针加了三个开关，用来在**同一条策略**上做物理 A/B（判断这是"策略钻空子"还是"仿真解不了接触"）：

```bash
# 基线（当前设置）→ 已实测：右手最深 −49.6 mm
JAX_PLATFORMS=cpu .venv-warp/bin/python src/check_grasp_penetration.py \
  --ckpt runs/ckpt_lboth_2bar_6h/actor_latest.pkl --goal-bar 2 --steps 250

# A) 关掉 MULTICCD（让 (CAPSULE,MESH) 不再是单接触对）
... --disable-multiccd
# B) 提高线搜索预算（场景为了速度钉死在 20）
... --ls-iterations 100
# C) 把接触做硬（timeconst 0.02 -> 0.002）
... --solref-timeconst 0.002
```

判据：**同一策略下，若某个设置把最小有符号距离拉回关键帧量级（≈ −5 mm）**，
就说明穿模是仿真设置问题，改场景后**必须重跑**（已有的 6 h 结果都不可用）；
若三个都不动，则说明是窄相/几何层面的问题（要考虑给手加显式 primitive 碰撞体或把杆换成别的截面）。

在这之前，**不要再基于 §9.79 的"成功"往下做目标设计**。

### 9.81 A/B 结论：穿模是**接触刚度**问题（不是策略、不是 MULTICCD、不是线搜索）

同一策略（`ckpt_lboth_2bar_6h/actor_latest`，`--goal-bar 2`，4 eps × 250 步）只改一个物理量：

| 设置 | 右手 min | 右手中位 | 右手 <−10 mm | 左手中位 | 结论 |
|---|---|---|---|---|---|
| **默认**（`ls=20`、MULTICCD on、`solref tc=0.02`）| −48.9 mm | −12.2 mm | **197/251** | +200 mm | ❌ |
| `--disable-multiccd` | −40.4 | −13.1 | 194 | +200 | ❌ 几乎没用 |
| `--ls-iterations 100` | −41.0 | −11.8 | 194 | +200 | ❌ 几乎没用 |
| `--solref-timeconst 0.002` | **−2.5** | **−0.7** | **0** | +200 | ✅ |
| **`--solref-timeconst 0.004`**（= 2×dt，推荐下限）| **−2.5** | **−0.7** | **0** | +200 | ✅ **最坏 −3.3 mm** |
| `--solref-timeconst 0.008` | −6.6 | −2.2 | 0 | +200 | ✅ 最坏 −7.9 mm |

* **只有接触刚度有效**：`0.02 → 0.004` 把最坏穿透从 **49.6 mm 降到 3.3 mm**（比关键帧在 0.02 下的 −5.1 mm 还浅）。
* MULTICCD / 线搜索**都不是原因**（改它们只动 5–5 mm）。
* **硬接触下这条策略不再移动**（两手最近杆都恒为 bar0 ⇒ 停在 B0）⇒ **那次"穿越到 B2"只在软接触下可能**。
  这也意味着**必须重训**：现有策略的"抓握"有一部分是"陷进去"。
* 修法（定向、安全）：只给**杆的 geom** 设 `solref="0.004 1"`（MuJoCo 的配对规则里非默认的一侧生效，
  且两者都非默认时取**更硬**的那个）⇒ 手-杆接触变硬，其他接触（脚/地面/自碰）不受影响。

#### 9.81.1 回答「为什么之前没有出现穿模」

1. **它一直都在，量级相同**。同一把尺子量旧策略：

   | 策略（都是"成功"的）| 承重手 | 中位 | 最深 | <−10 mm 步数 |
   |---|---|---|---|---|
   | `dual_hnext` 6 h @goal 1 | **左手** | **−9.4 mm** | **−39.4 mm** | **98/251（39%）** |
   | `support_dual` 6 h @goal 1 | 右手 | −8.3 mm | −33.8 mm | 32/251（13%）|
   | `load_both` 6 h @goal 2 | 右手 | −12.2 mm | −49.6 mm | 197/251（78%）|
   | `hang` 关键帧（人工标定）| — | — | −5.1 mm | — |

2. **我们的渲染尺度看不见**：相机 `--distance 2.8/3.2`、400 px、默认 fovy 45°
   ⇒ **5.8–6.6 mm/px**。于是 5 mm ≈ 0.9 px、10 mm ≈ 1.5 px（几乎不可见）、
   40–50 mm ≈ 6–9 px（明显，但只在少数帧）。旧 run 的中位穿透 8–9 mm 恰好落在"看不见"那一档。
3. **也从没量过**：我们所有的判据对这个现象都是盲的 —— `c`/`h` 是**座到轴的距离**（分不清"在杆外贴着"还是
   在杆里），`f` 是**指力**——而**指力正是穿透产生的**。所以穿模不但不被惩罚，反而会**把指标刷高**。
4. **新策略更严重**，因为 load 判据**奖励往里压**：压得越深 → 法向力越大 → `f` 越高；
   而纯距离判据只要"靠近"就满足了，没有往里压的激励。实测频率从 13–39% 升到 **78%**（前 250 步）。
   所以"用户在这一次才看到"既因为这一次更糟，也因为这一次是在**专门盯着那只承重手看一个刚出现的成功**。

#### 9.81.2 影响与下一步

* **所有已有 run 的"抓握"在物理合法性上都要打折扣**（包括 6 h 基线的 B1 悬挂，它中位就陷进 8.3 mm）。
  结论不是"没学到东西"，而是"**任务比真实任务更容易**"（可以陷进去当钩子）。
* 已把 `src/check_grasp_penetration.py` 作为新守卫（退出码可判），阈值 10 mm，并带 `--reference` 标定行。
* 建议顺序（每一步都要重新验收）：
  1. **改场景**：杆 geom 加 `solref="0.004 1"`（改 `assets/g1_brachiation/build_scene.py` 并重新导出 XML），
     跑 `check_scene` 确认 `nq/ngeom/bar_x` 未变；
  2. **重验 M1**：关键帧双手悬挂在新接触下是否仍能停住（`check_contact_sense` 的 P 臂 + `m3_swing`），
     以及 `check_grasp_penetration --reference`（关键帧在硬接触下应当 ≈ −1 mm）；
  3. **重训两杆线**（`support_dual` / `support_dual_load_both` 各 6 h 起），此时"存活/推进"才是真的；
  4. 守卫清单加 `check_grasp_penetration`（`docs/复现环境.md` §2.4 + `tools/setup_env.sh`）。
* **注意预期**：硬接触下任务**变难**（不能靠陷进去），所以短期指标会比现在的记录差，这是正常的。
